...
nrow(baseline) # Sample size 
range(baseline$age_at_study_date_x100/100) # Age range of surveyed participants 
...
end_new_names <- c("Asthma","PHD", "HF","RD", "MACE", "COPD", "Acute_MI", "IHD", "CVD")

end_pattern   <- c("ep_CKB0063", "ep_CKB0012", "ep_CKB0080", "ep_CKB0032", "ep_CKB0072", 
                   "ep_CKB0033", "ep_CKB0004", "ep_CKB0003", "ep_CKB0071")
...
# Drop missing values for BMI 
merged_data <- filter(merged_data, !is.na(bmi_calc))
      ...
      mutate(
         smoking_status = factor(
           case_when(smoking_category == 1 ~ "Never smoker", 
                     smoking_category == 2 ~ "Occasional", 
                     smoking_category == 3 ~ "Ex-smoker", 
                     TRUE ~ "Current")),              
             
         ever_reg_smoker = factor(ifelse(smoking_category >= 3, "Ever regular", "Never")),
...
n_var <- c("age_group10", "age", "edu_bin", "sex", "h_income_bin",
           "ever_reg_smoker", "current_drinkers", 
...
l_var <- c("Age group, year, n (%)", "Age, mean (SD), year", "Sex",
...
df_table <- dplyr::select(merged_data, all_of(n_var), hf_inpatient, phd_inpatient)

# 
# for (i in seq_along(n_var)) {
#   Hmisc::label(df_table[ ,n_var[i]]) <- l_var[[i]]
# }
...
tab_hf <-  descrTable(hf_inpatient ~ age + sex + area + edu_bin + h_income_bin + occupation +                   
                      ...
                      co_hf_phd + hf_phd_timing, df_table, hide.no = "No")
...
tab_phd <- descrTable(phd_inpatient ~ age + sex + area + edu_bin + h_income_bin + occupation +                   
                      ...
                      HTN + diabetes + chd + stroke_tia + copd + emphysema_bronchitis + CLD + tuberculosis +
                      asthma + emphy_bronc_asthma + SOB_walking + poor_health + 
                      co_hf_phd + phd_hf_timing, df_table, hide.no = "No")
...
tab_overall <- descrTable(df_table, hide.no = "No")
...
all_gender <- merged_data %>% select(sex, ever_reg_smoker, current_drinkers)
...
for (i in seq_along(c("Men", "Women"))) {
  ...
  tab_gen1[[i]] <-  
    ...
    descrTable(hf_inpatient ~ ever_reg_smoker + current_drinkers, 
      selec = list(ever_reg_smoker = sex == c("Men", "Women")[[i]], 
                   current_drinkers = sex == c("Men", "Women")[[i]]),
      df_table, hide.no = "No")
  
  tab_gen2[[i]] <- 
    
    descrTable(phd_inpatient ~ ever_reg_smoker + current_drinkers, 
      selec = list(ever_reg_smoker = sex == c("Men", "Women")[[i]],
                   current_drinkers = sex == c("Men", "Women")[[i]]), df_table, hide.no = "No")
  
  tab_gen3[[i]] <- 
    
    descrTable(all_gender,
      selec = list(
               ever_reg_smoker = sex == c("Men", "Women")[[i]],
               current_drinkers = sex == c("Men", "Women")[[i]]), 
      hide.no = "No")
...
data_expanded <- map(
  ...
  map(~make_quartile_var_labels( 
      dataset=.x, 
      numeric_vars=c("bmi_calc", "sbp_mean"), 
      n_quartile=4)
  )   
 
# Automating the process 
strata_var    <- c("ageatrisk65", "sex", "area")
grouping_var  <- c("edu", "sbp_mean_Q4", "bmi_calc_Q4", "diabetes", "copd", "chd")
group_labels  <- list(
...
for (df in seq_along(data_expanded)) {
  ...
  subgroup2_temp[[df]] <- map2(
      .x=grouping_var,
      .y=seq_along(grouping_var),  
      ~incidence_rates (
        data=data_expanded[[df]],
        group=.x, 
        strata=strata_var,
        per_pyar=100000) %>% 
      pluck (., 2) %>% 
  ...
  trend_het_test[[df]] <- map(
      .x = subgroup2_temp[[df]], 
      ~if(unique(.x$test_type) == "Trend"){
        trend(.x$std_ir, .x$std_ir_se) 
...
merged_data <- mutate(
  ...
  edu3=factor(case_when(
    highest_education==0~"None", 
    highest_education==1~"Primary", 
    TRUE~"Secondary or higher"))
  )
...
data_hf %>% 
  reframe(mean_age=mean(age_hf), sd_age=sd(age_hf), .by="sex")
              
data_phd <- 
  merged_data %>%
  filter(phd_inpatient==1) %>%
  mutate(
     futime_day=interval(date_phd_diag, censoring_date)/ddays(1),
     futime_yr=interval(date_phd_diag, censoring_date)/dyears(1),
     futime_age=(age_censor-age_phd)/365,
     age_diag_10=cut(age_phd,  
                     breaks=c(35, 50, 60, 70, 80, 100),
                     labels=c("30-49", "50-59", "60-69", "70-79", "80+")),
     age_diag65=factor(case_when(age_phd>=65~"65 or over", TRUE~"Below 65")),
...
data_phd %>% 
  reframe(mean_age=mean(age_phd), sd_age=sd(age_phd), .by="sex")


# Define outcome for 28-day CFR--
data_hf %>% 
  mutate(death_28=case_when(censoring_reason=="Dead" & futime_day<=28~1, TRUE~0)) -> data_hf        

data_phd %>% 
  mutate(death_28=case_when(censoring_reason=="Dead" & futime_day<=28~1, TRUE~0)) -> data_phd      
...
data_phd <- 
  data_phd %>%
  mutate(
     incident_copd_PHD=case_when(COPD_combined_ep==1 & COPD_combined_datedeveloped<=PHD_combined_datedeveloped~1, TRUE~0),
     develop_copd=factor(case_when(copd=="Yes" | incident_copd_PHD==1~"Yes", TRUE~"No"), levels = c("Yes", "No")),
     incident_MI_PHD=case_when(Acute_MI_combined_ep==1 & Acute_MI_combined_datedeveloped <= PHD_combined_datedeveloped~1, TRUE~0),
     incident_IHD_PHD=case_when(IHD_combined_ep==1 & IHD_combined_datedeveloped<=PHD_combined_datedeveloped~1, TRUE~0),
     develop_IHD=factor(case_when(
       chd=="Yes" | incident_MI_PHD==1 | incident_IHD_PHD==1~"Yes", TRUE~"No"), 
       levels = c("Yes", "No")),
     age_diag_merge=fct_recode(
        age_diag_10, 
        "30-59"="30-49", 
        "30-59"="50-59", 
        "60-69"="60-69", 
        "70-79"="70-79", 
        "80+"="80+"), 
     all_death=case_when(censoring_reason=="Dead"~1, TRUE~0)
